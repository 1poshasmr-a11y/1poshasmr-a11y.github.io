...
export const renderDashboardInventory = async (root) => {
    ...
    async function loadInventory(search = '') {
        try {
            const res = await fetch(SITE.api(`/equipment${search ? '?search=' + encodeURIComponent(search) : ''}`));
            ...
            window.deleteEquipment = async (id) => {
                if(confirm('Are you sure you want to delete this equipment?')) {
                    await fetch(SITE.api('/equipment/') + id, { method: 'DELETE' });
            ...
            tbody.innerHTML = equipment.map(item => `
                <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                    <td style="padding: 16px; width: 80px;">
                        <img src="${(item.images && item.images[0]) || item.image || ''}" alt="${item.title}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 4px;">
                    </td>
